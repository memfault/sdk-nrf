	...
	if (conn_err == 0) {
		if (count == 0) {
			err = memfault_metrics_heartbeat_timer_start(
					MEMFAULT_METRICS_KEY(ncs_bt_connection_time_ms));
			if (err) {
...
void memfault_bt_metrics_update(void)
{
	...
	err = memfault_metrics_heartbeat_set_unsigned(MEMFAULT_METRICS_KEY(ncs_bt_bond_count),
						      bond_count);
	if (err) {
		LOG_WRN("Failed to set the ncs_bt_bond_count metric, err: %d", err);
	}

	err = memfault_metrics_heartbeat_set_unsigned(MEMFAULT_METRICS_KEY(ncs_bt_connection_count),
